...
namespace Anaglyph.XRTemplate.DepthKit
{
	...
	public class DepthKitDriver : MonoBehaviour
	{
		...
		private void Start()
		{
			Debug.Log("[DepthKitDriver] Start() – driver is alive");
		...
		public void UpdateCurrentRenderingState()
		{
			...
			bool isNowAvailable = depthTex != null;

			// 🔹 Log the transition for debugging
			if (isNowAvailable != DepthAvailable)
			{
				DepthAvailable = isNowAvailable;
				Debug.Log($"[DepthKitDriver] DepthAvailable = {DepthAvailable}, tex = {depthTex}");
			}
